...
function pageReady() {
    ...
    * Sunday = index 3*/
	...
	//highlights store hours for all restaurant locations
	...
	function showMap() {
	  ...
	  var toronto_restaurant = {lat: 43.658745, lng:-79.378888};
	  var richmondHill_restaurant = {lat: 43.894492, lng:-79.465351};
	  var vaughan_restaurant = {lat: 43.825730, lng: -79.538244};
	  ...
	  //Adds event listener for all markers, store information briefs, and information icons
	  for(var i=0; i<restaurant_markers.length; i++){
	      info_icons[i].onclick = viewStoreDetails;
	      store_brief_info[i].onclick = viewLocation;
	      store_brief_info[i].onmouseover = highlightStoreInfo;
	...
	//When marker is clicked, zooms in on the location, and centers map on the location marker
	  function viewStoreDetails(){
	    for(var i=0; i<restaurant_markers.length; i++){
	      if(this===info_icons[i]){
	        restaurant_location_information[i].scrollIntoView({behavior: 'smooth' });
	      }
	    }
	  }
